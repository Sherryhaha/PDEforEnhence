...
void PMenhence::pm1_diffusion(Mat&A, Mat&B, double k)	//按照第一个公式求扩散系数'pm1': perona-malik, c=exp{-(|grad(J)|/K)^2} [PM90]
{
    int h,w;
    for( h=0; h<Y_image ; h++)
        for(w=0; w<X_image ; w++)
...
void PMenhence::pm2_diffusion(double *A, double *B, double k)	//按照第二个公式求扩散系数'pm2': perona-malik, c=1/{1+(|grad(J)|/K)^2} [PM90]
{
    int h,w;
    for(h=0; h<Y_image ;h++)
        for(w=0; w<X_image ;w++)
    ...
    for(n=0;n<(loop);n++)		//叠代计算
    {
        for( h=1; h<Y_image-1 ;h++)		//计算各个方向梯度
            for(w=1; w<X_image-1 ;w++)
            {
                ...
                sum=sum*dt;		//乘时间增量因子
                IMG[h*X_image+w]=IMG[h*X_image+w]+sum;	//更新图像数据
            }
    }
...
//    printMat(src);
    ...
    p.DiffusionPic(unsigned char *img, int method, int feed,int loop, double K, double dt)
